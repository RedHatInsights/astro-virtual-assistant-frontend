...
export const AstroChat: React.FunctionComponent<AstroChatProps> = ({
  ...
  const switchMessages = useMemo(() => {
    return messages.map((message, index) => {
      ...
      switch (message.from) {
        case From.ASSISTANT:
          return <AssistantMessageEntry message={message} ask={askFromOption} preview={preview} blockInput={blockInput} showThumbs={mostRecentMessage && !firstMessage} key={index} />;
        case From.USER:
