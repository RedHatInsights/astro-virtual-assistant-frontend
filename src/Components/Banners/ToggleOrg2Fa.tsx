...
export const ToggleOrg2FaBanner: FunctionComponent<MessageProps<Banner>> = ({ message }) => {
  ...
  const twoFaDocsHref: string = "https://access.redhat.com/documentation/en-us/red_hat_customer_portal/1/html/using_two-factor_authentication/index"
  return (
    <>
        ...
        <ConversationAlert variant="success" title={`Two-factor authentication ${enableOrg2fa ? "enabled" : "disabled"} successfully`}>
          {
            enableOrg2fa ?
              <div>
                <p>Two-factor authentication has been enabled successfully. Users will be required to set up two-factor authentication the next time they attempt to log in.</p>
              <p>They can chat with me if they need help setting up two-factor authentication or visit <a href={twoFaDocsHref} target="_blank" rel="noopener noreferrer">our documentation</a>.</p>
              </div> :
